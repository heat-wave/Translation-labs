...
import task4.grammar.GrammarBaseListener;
import task4.grammar.GrammarLexer;
import task4.grammar.GrammarParser;
import task4.utils.Node;
import task4.utils.Production;
...
import static task4.ExecutionMode.DEBUG;
...
public class ParserGenerator {
    ...
    public ParserGenerator(ExecutionMode mode, File outputDir) {
        this.mode = mode;
        this.outputDir = outputDir;
    }

    public void prepare(File grammarFile, String startRule) throws IOException {
    ...
    private void generateLexer() throws IOException {
        ...
        out.println("\nimport java.io.IOException;");
        out.println("import java.io.PushbackInputStream;");
        out.println("import java.text.ParseException;\n");

        out.println("public class " + LEXER_NAME + " {");
        out.println("\tprivate PushbackInputStream is;");
        ...
        out.println("\tpublic " + LEXER_NAME + "(PushbackInputStream is) throws ParseException, IOException {");
        ...
        for (String curStringTerminal : terminals.keySet()) {
            for (Production productionString : terminals.get(curStringTerminal).getProductionList()) {
                out.println(String.format(
                                "        if" +
                                " (!match && \"%1$s\".startsWith(String.valueOf((char) curChar))) {\n" +
                                "            while(true) {\n" +
                                "                 curString += (char) curChar;\n" +
                                "                 if(curString.length() == \"%1$s\".length()) break;\n" +
                                "                 nextChar();\n" +
                                "            }\n" +
    ...
    private void generateParser() throws IOException {
        ...
        out.println("import task4.utils.Tree;\n");
        ...
        out.println("\n\tpublic " + start.getReturnType() + " parse(PushbackInputStream is" + (start.getDeclAttrs(true).isEmpty() ? "" : ", " + start.getDeclAttrs(true)) + ") throws ParseException, IOException {");
